...
def train_random_forest_wav(df: pd.DataFrame):
    df = preprocess_wav(df)

    X = np.array(df["rms"].tolist())
    y_form = df["form"]
    ...
    with mlflow.start_run():
        ...
        combined_model_path = os.path.join("models/", "random_forest_models_wav.pkl")
